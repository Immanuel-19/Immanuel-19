## Hi, I'm Immanuel!👋

Passionate about turning raw data into meaningful insights that drive better business decisions, I use data to help businesses make better decisions.

My goal is to help organizations make decisions based on data by providing accurate insights, clear recommendations, and easy-to-navigate dashboards. These tools support planning and help improve business performance. Ready to learn, work with others, and use data to create measurable results.

--- 

 ### 🧠 Data Analytics Skills
- Requirements gathering
- Critical thinking & problem-solving
- Stakeholder management
- Process improvement
- Communication & presenting insights to non-technical audiences

### 🛠️ Technical Tools
- **Excel** (Advanced — Pivot Tables, Power Query, VBA, Dashboards)
- **SQL** (Building proficiency)
- **Power BI** (Building proficiency)
- **Python**

### 🌱 Currently Learning
SQL → Power BI → Python, building toward becoming a well-rounded analyst who can move fluidly between business strategy and technical execution.


<h2> 🤳 Connect with me:</h2>

Open to opportunities and conversations around business analysis, data-driven decision making, and helping businesses grow.

[<img align="left" alt="ImmanuelAhanonu | Twitter" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/twitter.svg" />][twitter]
[<img align="left" alt="ImmanuelAhanonu | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]

[twitter]: https://x.com/immanuelahanonu
[linkedin]: www.linkedin.com/in/immanuel-ahanonu-39b950405
